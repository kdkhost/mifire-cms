        ...
        <div class="max-w-2xl space-y-6">
                    ...
                    <div x-data="{ preview: '{{ $user->avatar ? asset('storage/' . $user->avatar) : '' }}' }">
                        ...
                        <div class="flex items-center gap-4">
                            <div
                                ...
                                <template x-if="!preview">
                                    <div
                                        class="w-full h-full bg-red-100 flex items-center justify-center text-red-600 font-bold text-xl">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <p class="text-gray-700 mt-0.5">
                                {{ $user->last_login_at ? $user->last_login_at->format('d/m/Y H:i') : 'Nunca' }}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <form action="{{ route('admin.users.change-password', $user) }}" method="POST">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="new_password" class="block text-sm font-medium text-gray-700 mb-1">Nova Senha <span
                                    class="text-red-500">*</span></label>
                            <input type="password" name="password" id="new_password" required minlength="8"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500">
                            @error('password', 'changePassword') <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            <label for="new_password_confirmation"
                                class="block text-sm font-medium text-gray-700 mb-1">Confirmar Nova Senha <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                class="px-6 py-2 bg-gray-800 text-white text-sm font-medium rounded-lg hover:bg-gray-900 transition-colors shadow-sm">
                                Alterar Senha
                            </button>
                        </div>
                    </div>
