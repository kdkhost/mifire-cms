...
<div x-data="menuForm()">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Editar Item de Menu</h1>
            <p class="text-sm text-gray-500 mt-1">{{ $menu->title }}</p>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <div x-show="linkType === 'url'">
                        <input type="url" name="url" value="{{ old('url', $menu->url) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="target" id="target"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500">
                            <option value="_self" {{ old('target', $menu->target) === '_self' ? 'selected' : '' }}>Mesma aba</option>
                            <option value="_blank" {{ old('target', $menu->target) === '_blank' ? 'selected' : '' }}>Nova aba</option>
                        </select>
...
<script>
    function menuForm() {
        return {
            isActive: {{ old('is_active', $menu->is_active) ? 'true' : 'false' }},
            linkType: '{{ old('link_type', $menu->page_id ? 'page' : 'url') }}'
        }
    }
</script>
@endpush
