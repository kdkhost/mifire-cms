...
<div x-data="bannerForm()">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ $banner->title }}</p>
        </div>
        <a href="{{ route('admin.banners.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500">{{ old('description', $banner->description) }}</textarea>
                        @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="button_text" class="block text-sm font-medium text-gray-700 mb-1">Texto do Botão</label>
                        ...
                        <div>
                            <label for="button_url" class="block text-sm font-medium text-gray-700 mb-1">URL do Botão</label>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wider mb-3">Imagem do Banner</h3>
                    <div x-data="{ preview: '{{ $banner->image ? asset('storage/' . $banner->image) : '' }}' }"
                        ...
                        <label class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-red-400 hover:bg-red-50/50 transition-colors"
                               x-show="!preview">
...
<script>
    function bannerForm() {
        return { isActive: {{ old('is_active', $banner->is_active) ? 'true' : 'false' }} }
    }
</script>
@endpush
@endsection
