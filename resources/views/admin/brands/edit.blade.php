...
<div x-data="brandForm()">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.brands.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
        ...
        <div class="max-w-2xl space-y-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="url" name="url" id="url" value="{{ old('url', $brand->url) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500"
                ...
                <div>
                    ...
                    <div x-data="{ preview: '{{ $brand->logo ? asset('storage/' . $brand->logo) : '' }}' }">
                        <template x-if="preview">
                            <div class="relative mb-3 inline-block">
                                <img :src="preview" class="h-24 object-contain rounded-lg border border-gray-200 p-2">
                                <button type="button" @click="preview = ''; $refs.logoInput.value = ''"
                        ...
                        <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-red-400 hover:bg-red-50/50 transition-colors"
                            ...
                            <input type="file" name="logo" accept="image/*" class="hidden" x-ref="logoInput"
                                   @change="
...
<script>
    function brandForm() {
        return { isActive: {{ old('is_active', $brand->is_active) ? 'true' : 'false' }} }
    }
</script>
@endpush
@endsection
