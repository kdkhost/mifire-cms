    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                    <div>
                        <label for="short_description" class="block text-sm font-medium text-gray-700 mb-1">Descrição Curta</label>
                        <textarea name="short_description" id="short_description" rows="3"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                  placeholder="Breve descrição do produto...">{{ old('short_description', $product->short_description) }}</textarea>
                        @error('short_description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Descrição Completa</label>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="meta_description" id="meta_description" rows="2"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                  placeholder="Descrição para SEO" maxlength="160">{{ old('meta_description', $product->meta_description) }}</textarea>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div x-data="{ preview: '{{ $product->image ? asset('storage/' . $product->image) : '' }}' }"
                        ...
                        <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-red-400 hover:bg-red-50/50 transition-colors"
                            ...
                            <input type="file" name="image" accept="image/*" class="hidden" x-ref="mainImage"
                                   @change="
                                       const reader = new FileReader();
                                       reader.onload = (e) => preview = e.target.result;
                    ...
                    @if($product->gallery && count($product->gallery) > 0)
                        <div class="grid grid-cols-3 gap-2 mb-3">
                            @foreach($product->gallery as $img)
                                <img src="{{ asset('storage/' . $img) }}" alt="Gallery" class="w-full h-20 object-cover rounded-lg">
                            @endforeach
                        </div>
                        <p class="text-xs text-gray-400 mb-2">Enviar novas imagens substituirá as atuais</p>
                    @endif
                    <div x-data="{ previews: [] }">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    @if($product->datasheet_url)
                        <a href="{{ asset('storage/' . $product->datasheet_url) }}" target="_blank"
                           class="inline-flex items-center gap-2 text-sm text-red-600 hover:text-red-700 mb-3">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            Ver datasheet atual
                        </a>
                    @endif
                    <label class="flex flex-col items-center justify-center w-full h-20 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-red-400 hover:bg-red-50/50 transition-colors">
                        <svg class="w-6 h-6 text-gray-400 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
...
<script>
    function productForm() {
        const existingSpecs = @json(old('specifications', $product->specifications ?? []));
        let specs = [];
        if (Array.isArray(existingSpecs)) {
            specs = existingSpecs.map(s => ({ key: s.key || '', value: s.value || '' }));
        } else if (typeof existingSpecs === 'object' && existingSpecs !== null) {
            specs = Object.entries(existingSpecs).map(([key, value]) => ({ key, value }));
        }
        if (specs.length === 0) specs = [{ key: '', value: '' }];

        return {
            isActive: {{ old('is_active', $product->is_active) ? 'true' : 'false' }},
            isFeatured: {{ old('is_featured', $product->is_featured) ? 'true' : 'false' }},
            specs,
            addSpec() {
