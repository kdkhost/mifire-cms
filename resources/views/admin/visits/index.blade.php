...
<div x-data="visitsPage()">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Acompanhe o tráfego do site</p>
        </div>
        <div class="flex items-center gap-3">
            <form method="GET" class="flex items-center gap-2">
                ...
                <input type="date" name="date_to" value="{{ request('date_to') }}"
                       class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500">
                <button type="submit" class="px-4 py-2 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 transition-colors">Filtrar</button>
            </form>
            <a href="{{ route('admin.visits.export') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition-colors shadow-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($visits as $visit)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-3 text-gray-900 max-w-xs truncate" title="{{ $visit->url }}">{{ $visit->url }}</td>
                            ...
                            <td class="px-6 py-3 text-gray-600">
                                ...
                                @elseif($visit->device === 'tablet')
                                    <span class="inline-flex items-center gap-1 text-xs bg-purple-50 text-purple-700 px-2 py-1 rounded-full">
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1 text-xs bg-gray-50 text-gray-700 px-2 py-1 rounded-full">
                            ...
                            <td class="px-6 py-3 text-gray-600">{{ $visit->city ? $visit->city . ', ' . $visit->country : ($visit->country ?? '-') }}</td>
                            <td class="px-6 py-3 text-gray-500 whitespace-nowrap">{{ $visit->created_at->format('d/m/Y H:i') }}</td>
                        </tr>
                    @empty
                        <tr>
