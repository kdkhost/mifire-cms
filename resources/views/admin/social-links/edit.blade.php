        ...
        <form action="{{ route('admin.social-links.update', $socialLink) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="max-w-2xl space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                    <div>
                        ...
                        <select name="platform" id="platform" required
                            ...
                            @foreach(['facebook', 'instagram', 'twitter', 'linkedin', 'youtube', 'tiktok', 'pinterest', 'whatsapp', 'telegram', 'github', 'outro'] as $p)
                                <option value="{{ $p }}" {{ old('platform', $socialLink->platform) === $p ? 'selected' : '' }}>
                                    {{ ucfirst($p) }}</option>
                            @endforeach
                        </select>
                    ...
                    <div
                        x-data="{ uploadType: '{{ $socialLink->icon && (Str::contains($socialLink->icon, '.') || Str::startsWith($socialLink->icon, 'http')) ? 'file' : 'class' }}' }">
                        ...
                        <div x-show="uploadType === 'file'" x-cloak>
                            <div class="flex items-center gap-4 mb-2">
                                @if($socialLink->icon && (Str::contains($socialLink->icon, '.') || Str::startsWith($socialLink->icon, 'http')))
                                    <div
                                        class="w-10 h-10 bg-gray-100 rounded flex items-center justify-center overflow-hidden border">
                                        <img src="{{ Str::startsWith($socialLink->icon, 'http') ? $socialLink->icon : asset('storage/' . $socialLink->icon) }}"
                                            class="w-6 h-6 object-contain">
                                    </div>
                                @endif
                                <div class="flex-1">
                                    <label for="icon_file" class="block text-sm font-medium text-gray-700 mb-1">Alterar
                                        Ícone</label>
                                    <input type="file" name="icon_file" id="icon_file" accept="image/*"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500">
                                </div>
                            </div>
                            <p class="text-xs text-gray-400">Formatos aceitos: SVG, PNG, JPG (Máx. 1MB)</p>
                        </div>
                    </div>
